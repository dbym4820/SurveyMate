 ...
 * スケジューラーの状態を取得
 */
router.get('/scheduler/status', adminOnly, async (_req: Request, res: Response): Promise<void> => {
  try {
 ...
 * 手動でRSS取得を実行
 */
router.post('/scheduler/run', adminOnly, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: (error as Error).message || 'Fetch failed' });
  }
});
 ...
 * 取得ログを取得
 */
router.get('/logs', adminOnly, async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 * 論文誌を追加（認証済みユーザー全員）
 */
router.post('/journals', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id, name, fullName, publisher, rssUrl, category, color } = req.body as {
      id?: string;
      name?: string;
      fullName?: string;
      publisher?: string;
 ...
 */
router.post('/journals/test-rss', async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
router.get('/journals/:id/fetch', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 * 論文誌を更新（認証済みユーザー全員）
 */
router.put('/journals/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { name, fullName, publisher, rssUrl, category, color, isActive } = req.body as {
      name?: string;
      fullName?: string;
      publisher?: string;
      rssUrl?: string;
 ...
 */
router.delete('/journals/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 * 論文誌を再有効化（認証済みユーザー全員）
 */
router.post('/journals/:id/activate', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 * ユーザー一覧を取得（管理者のみ）
 */
router.get('/users', adminOnly, async (_req: Request, res: Response): Promise<void> => {
  try {
    const sql = `
