...
export async function query<T extends RowDataPacket[] | ResultSetHeader>(
  ...
  try {
    ...
    logger.error('Database query error:', { sql, error: (error as Error).message });
    throw error;
  }
}
...
export const papers = {
   ...
   * 論文を挿入または更新（重複チェック付き）
   */
  async upsert(paper: PaperInput): Promise<ResultSetHeader> {
    const sql = `
   ...
   * 論文一覧を取得
   */
  async findAll(params: PapersListParams = {}): Promise<Paper[]> {
    ...
    const rows = await query<Paper[]>(sql, queryParams);
    return rows.map(row => ({
      ...row,
   ...
   * 論文を1件取得
   */
  async findById(id: number | string): Promise<Paper | null> {
    const sql = `
      SELECT
        p.*,
        j.name AS journal_name,
      ...
      WHERE p.id = ?
    `;
    const rows = await query<Paper[]>(sql, [id]);
   ...
   * 論文数を取得
   */
  async count(params: Omit<PapersListParams, 'limit' | 'offset'> = {}): Promise<number> {
...
export const journals = {
   ...
   * 全論文誌を取得
   */
  async findAll(activeOnly = true): Promise<Journal[]> {
    let sql = `
      SELECT j.*,
  ...
  async findById(id: string): Promise<Journal | null> {
    ...
    const rows = await query<Journal[]>(sql, [id]);
    return rows[0] || null;
  },
   ...
   * 最終取得日時を更新
   */
  async updateLastFetched(id: string): Promise<ResultSetHeader> {
    const sql = `UPDATE journals SET last_fetched_at = CURRENT_TIMESTAMP WHERE id = ?`;
    return query<ResultSetHeader>(sql, [id]);
  },
};
...
export const summaries = {
   ...
   * 要約を作成
   */
  async create(summary: SummaryInput): Promise<Summary & { id: number }> {
    const sql = `
    ...
    return { id: result.insertId, ...summary } as Summary & { id: number };
  },

  /**
   * 論文IDで要約を取得
   */
  async findByPaperId(paperId: number | string): Promise<Summary[]> {
    const sql = `SELECT * FROM summaries WHERE paper_id = ? ORDER BY created_at DESC`;
    return query<Summary[]>(sql, [paperId]);
  },
};
...
export const users = {
  ...
  async create(user: {
    ...
    email: string | null;
    is_admin?: boolean;
  }): Promise<{ id: number; username: string; email: string | null; is_admin: boolean }> {
   ...
   * 最終ログイン更新
   */
  async updateLastLogin(id: number): Promise<ResultSetHeader> {
...
export const sessions = {
   ...
   * セッション作成
   */
  async create(sessionId: string, userId: number, expiresAt: Date): Promise<ResultSetHeader> {
    const sql = `INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)`;
    return query<ResultSetHeader>(sql, [sessionId, userId, expiresAt]);
  },
  ...
  async findById(sessionId: string): Promise<Session | null> {
    ...
    `;
    const rows = await query<Session[]>(sql, [sessionId]);
    return rows[0] || null;
  },
   ...
   * セッション削除
   */
  async delete(sessionId: string): Promise<ResultSetHeader> {
    const sql = `DELETE FROM sessions WHERE id = ?`;
    return query<ResultSetHeader>(sql, [sessionId]);
  },
  ...
  async deleteByUserId(userId: number): Promise<ResultSetHeader> {
    ...
    return query<ResultSetHeader>(sql, [userId]);
  },
};
...
export const fetchLogs = {
   ...
   * ログを作成
   */
  async create(log: FetchLogInput): Promise<FetchLog & { id: number }> {
    const sql = `
    ...
    return { id: result.insertId, ...log } as FetchLog & { id: number };
  },

  /**
   * 最新ログを取得
   */
  async findRecent(limit = 50): Promise<FetchLog[]> {
    const sql = `
      ...
      LIMIT ?
    `;
    return query<FetchLog[]>(sql, [limit]);
  },
};
