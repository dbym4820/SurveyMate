 ...
 * 単一の論文誌からRSSを取得
 */
async function fetchJournalFeed(journal: Journal): Promise<FetchResult> {
  ...
  try {
    ...
    for (const item of feed.items) {
      ...
      // DOIの抽出
      let doi: string | null = item.doi || null;
      if (!doi && item.link) {
      ...
      try {
        ...
        if (!(error as Error).message.includes('Duplicate')) {
          logger.warn(`Failed to insert paper: ${paper.title}`, { error: (error as Error).message });
        }
      }
    }
    ...
    logger.error(`Failed to fetch: ${journal.name}`, { error: (error as Error).message });
 ...
 * 次回実行時間を更新
 */
function updateNextRunTime(schedule: string): void {
 ...
 * 手動実行
 */
export async function runNow(): Promise<FetchAllResult> {
 ...
 * 特定の論文誌のみ取得
 */
export async function fetchJournal(journalId: string): Promise<FetchResult> {
