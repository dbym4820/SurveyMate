...
// ヘルスチェック
router.get('/api/health', (_req: ExpressRequest, res: Response) => {
  res.json({
    status: 'ok',
...
// フロントエンドルーティング（SPA対応）
router.get('*', (req: ExpressRequest, res: Response) => {
  // APIリクエストは404を返す
...
const errorHandler: ErrorRequestHandler = (err: HttpError, _req: ExpressRequest, res: Response, _next: NextFunction): void => {
